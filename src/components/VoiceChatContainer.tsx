...
function VoiceChatContainer() {
  ...
  const { room: liveKitRoomFromHook, connectionState: liveKitActualStateFromHook } = useLiveKitRoom({
    liveKitUrl: process.env.NEXT_PUBLIC_LIVEKIT_URL || '',
    token: liveKitToken,
    onConnected: (room) => {
      console.log('[VCContainer] Hook onConnected. Sala conectada.');
      setConnectionState(LiveKitConnectionState.Connected);
      clearError();
    },
    onDisconnected: (reason) => {
      console.log('[VCContainer] Hook onDisconnected. Razón:', reason);
      setConnectionState(LiveKitConnectionState.Disconnected);
      if (conversationActive) {
        setAppError('livekit', 'Desconectado del chat de voz.');
      }
    },
    ...
    onConnectionError: (error) => {
      console.error('[VCContainer] Hook onConnectionError:', error);
      setConnectionState(LiveKitConnectionState.Disconnected);
      setAppError('livekit', error instanceof Error ? error.message : 'Error desconocido al conectar con LiveKit.');
    }
  });

  useEffect(() => { roomRef.current = liveKitRoomFromHook; }, [liveKitRoomFromHook]);
  ...
  }, [authStatus, session?.user?.id, userProfile, initialContext, getLiveKitToken]);
